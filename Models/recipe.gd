...
func canCraft(item1: Item, item2: Item):
	if item1.name == self.item1.name:
		if item2.name == self.item2.name:
			return item1.amount >= self.item1cost && item2.amount >= self.item2cost
	elif item2.name == self.item1.name:
		if item1.name == self.item2.name:
...
func craft(item1, item2):
	...
	var dependendOnInputItems = self.resultAmount.split("|")[0] == "+"
	if dependendOnInputItems:
		...
		if self.ratio == "1:1":
			...
			res.amount = (temp1.amount + temp2.amount) * int(resultAmount.split("|")[1])
			return {"res": res, "item1used": temp1.amount, "item2used": temp2.amount}
